...
**Agentic event store & enterprise audit infrastructure** — immutable memory and governance backbone for multi-agent AI systems at production scale.

# **Why this system**

Enterprise AI systems often lack **shared, persistent, append-only memory**: traces are ephemeral, judgements are lost when processes end, and governance hooks do not compose into a single auditable history.

**The Ledger** is the event store that downstream products integrate with so that: agent decisions are auditable and reproducible; temporal queries support compliance and debugging; the append-only ledger avoids the “lost context” failure mode (Gas Town); and consumers get a typed, queryable API.

Regulators and risk teams require an **immutable record** of every AI decision and the data behind it. The Ledger is that record.
...
* Translate “we need auditability” into a concrete event-store deployment approach.  
* Map the PostgreSQL implementation to common enterprise stacks (e.g. Marten/Wolverine, EventStoreDB) where clients have preferences.  
* Communicate migration cost and long-term commitment of event sourcing accurately.  
* Define **SLO-based** behaviour (latency, lag) rather than “as fast as possible.”

# **Operational demo standard**

The system should support: *“Show me the complete decision history of application ID X”* — from first event to final decision, including every agent action, compliance check, human review, causal links, temporal query at any lifecycle point, and cryptographic integrity verification — **runnable end-to-end in under 60 seconds** for operations and demos.
...
| Kafka \+ Kafka Streams | Any | Very-high-throughput event streaming; not a true event store (retention limits) | Ubiquitous in large enterprise | INTEGRATION — connect The Ledger to Kafka via the Outbox pattern |
...
Produce `DOMAIN_NOTES.md` before large implementation. Answer all of the following with specificity, not generality. Use it to align the team on tradeoffs before code hardens.
...
This phase is optional and demanding. Attempt it only after Phases 1–5 are solid.
...
Architecture is about tradeoffs. A decision without tradeoff analysis is a default, not a decision. Recommended sections:
...
6. **What you would do differently:** Name the single most significant architectural decision you would reconsider with another full day — distinguish “what we built” from “the best version of this.”

# **Component inventory**

The codebase may use `ledger/` instead of a generic `src/` tree — see `spec/mappings/repo-layout-mapping.md`.

**Core:** schema (`events`, `event_streams`, `projection_checkpoints`, `outbox`), `EventStore`, Pydantic event models, aggregates (loan, agent session, compliance, audit), command handlers, projections + daemon, upcasting + integrity + Gas Town, MCP tools and resources, optional what-if and regulatory package.

**Tests:** concurrency / double-decision, upcasting immutability, projections + lag, Gas Town recovery, MCP lifecycle — see `spec/09-documentation/test-strategy.md` and `tests/`.

**Documentation:** `DOMAIN_NOTES.md`, `DESIGN.md` (sections above), architecture diagrams, runbooks — see `spec/09-documentation/`.
